...
class CONFIGURE:
    ...
    def write_run_config(self):
        ...
        #add telomere secuence for manual curation if -c or --curation
        if self.args.curation:
            if self.args.telomere_sequence != None and len(self.args.telomere_sequence) >= 4:
                self.run_config["telomeres"] = self.args.telomere_sequence
            else:
                print("The telomere sequence must be a DNA sequence with at least 4 letters.")
                sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        ...
        "--telomere_sequence",
        default=None,
        help="Telomere sequence used to generate a telomere track only if usinng manual curation mode",
    )
